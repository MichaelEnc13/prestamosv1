...
class User{
    ...
    public static function session($userName, $pass){
        ...
        $check = User::check($uMail = "",$userName ); //verifica si existe el usuario
        if($check != ""):
    ...
    public static function mailer($uMail,$userName,$subject,$body,$alt = ""){ // usada para enviar correos  a los usuarios
        ...
        $mail = new PHPMailer(true);
            ...
            $mail   -> Port = 465 ;
            //receptores
            $mail -> setFrom("mail@dotsell.website","Dotsell System");
    ...
    public static function sentMail($c,$user){
        $cmd = "UPDATE usuarios SET sent_mail =  ? WHERE user_id = ?";
        $cmd = Conn::conect() -> prepare($cmd);
        $done = $cmd -> execute(array(
          $c,$user
        ));
